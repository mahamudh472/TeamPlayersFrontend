...
export const Button: React.FC<ButtonProps> = ({
  ...
  const baseStyles =
    "inline-flex items-center justify-center font-semibold rounded-lg transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary cursor-pointer active:scale-98 disabled:opacity-50 disabled:pointer-events-none";
  ...
  const sizes = {
    sm: variant === "icon" ? "h-8 w-8 p-1.5" : "px-3 py-1.5 text-xs gap-1.5",
    md: variant === "icon" ? "h-10 w-10 p-2" : "px-4 py-2 text-sm gap-2",
    lg: variant === "icon" ? "h-12 w-12 p-2.5" : "px-6 py-3 text-base gap-2.5",
  };
  ...
  return (
    ...
    >
      ...
      {children}
      {!loading && SuffixIcon && <SuffixIcon className={`${iconSizes[size]} shrink-0`} />}
    </button>
  );
};
